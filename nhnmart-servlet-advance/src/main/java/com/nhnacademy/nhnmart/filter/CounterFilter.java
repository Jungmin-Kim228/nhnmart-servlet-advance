...
@WebListener
@Slf4j
public class CounterFilter implements ServletContextListener {
    @Override
    public void contextInitialized(ServletContextEvent sce) {
        ...
        try (DataInputStream dis = new DataInputStream(
            servletContext.getResourceAsStream(countFileName))) {
            counter = dis.readInt();
        } catch (IOException e) {
            log.error("", e);
        }
    ...
    public void contextDestroyed(ServletContextEvent sce) {
        ...
        try (FileOutputStream fos = new FileOutputStream(
            new File(servletContext.getResource(countFileName).toURI()))) {
